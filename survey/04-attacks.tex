...
%%% Pinocchio Coin
Pinocchio Coin \cite{pinocchio} is an attempt at optimizing the Zerocoin protocol. Whereas Zerocoin uses the Strong RSA assumptions with double discrete logarithm proofs, Pinocchio Coin uses elliptic curves and bilinear pairings. Pinocchio Coin follows the same process as Zerocoin.  Initial parameters for the elliptic curve are set up. Pinocchio Coin also has users secretly committing Bitcoins. When the user wants to spend Bitcoins, the user creates a Pinocchio proof of work using quadratic programs.

The Pinocchio proof of work was originally designed as a generic proof of work to be used in applications such as cloud computing. The system takes a set of operations and converts them into proof of work that can be seen as a zero knowledge proof. The advantages of using Pinocchio Coin over Zerocoin is that the size of the proof is much smaller. The Pinocchio proofs are less than 400 bytes as opposed to Zerocoin's 50kB proofs. The potential disadvantages of using Pinocchio Coin is that there is no security analysis as of yet. Futhermore, although Pinocchio serves as an efficient general proof compiler, it is unknown if specialized systems, such as ZKPDL, would exhibit better performance in the Zerocoin framework.
